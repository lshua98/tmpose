# ------------------------------------------------------------------------------
# Modified by Shaohua Li (lshua98@163.com)
# ------------------------------------------------------------------------------
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask = None,pos=None):
        B, N, L = x.shape
        pos = pos[:,0:int(N-self.num_keypoints),:]
...
class TokenPose_TB_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, apply_multi=True, hidden_heatmap_dim=64*6,heatmap_dim=64*48,heatmap_size=[64,48], channels = 3, dropout = 0., emb_dropout = 0., pos_embedding_type="learnable", sample_type, sample_ratio):
        super().__init__()
    ...
    def _token_masked(self, x, ratio, sampled_type):

        # 获取 x 的形状信息
        batch_size, num_tokens, token_len = x.shape

        # 计算采样后的新的 token 数量
        new_num_tokens = int(num_tokens * ratio)

        # 在每个 token 中采样
        for b in range(batch_size):
            if sampled_type == "random":  # 随机采样
                random_sample = np.random.choice(num_tokens, size=new_num_tokens, replace=False)
                sampled_x = x[:, random_sample, :]
            if sampled_type == "uniform":  # 均匀采样
                uniform_sample = np.linspace(0, num_tokens - 1, new_num_tokens, dtype=int)
                sampled_x = x[:, uniform_sample, :]
            if sampled_type == "block":  # 片区采样
                num_zones = 4  # 示例：将256个token分为4个片区
                zone_size = int(256 * ratio / num_zones)  # 计算每个片区的大小
                sampled_x = torch.zeros_like(x)[:,:(zone_size * num_zones),:] # 创建采样后的变量，初始化为全零
                for i in range(num_zones):
                    start_idx = i * zone_size  # 计算当前片区的起始索引
                    end_idx = start_idx + zone_size  # 计算当前片区的结束索引
                    sampled_x[:, start_idx:end_idx, :] = x[:, start_idx:end_idx, :]  # 进行片区采样
        return sampled_x
    ...
    def forward(self, feature, mask = None):
        ...
        x = self._token_masked(x,self.ratio,self.sampled_type)
...
class TokenPose_L_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, hidden_heatmap_dim=64*6,heatmap_dim=64*48,heatmap_size=[64,48], channels = 3, dropout = 0., emb_dropout = 0.,pos_embedding_type="learnable", sample_type, sample_ratio):
        super().__init__()
        assert isinstance(feature_size,list) and isinstance(patch_size,list), 'image_size and patch_size should be list'
        assert feature_size[0] % patch_size[0] == 0 and feature_size[1] % patch_size[1] == 0, 'Image dimensions must be divisible by the patch size.'
        num_patches = (feature_size[0] // (patch_size[0])) * (feature_size[1] // (patch_size[1]))
    ...
    def _token_masked(self, x, ratio, sampled_type):
        ...
        for b in range(batch_size):
            ...
            if sampled_type == "uniform":  # 均匀采样
                uniform_sample = np.linspace(0, num_tokens - 1, new_num_tokens, dtype=int)
                sampled_x = x[:, uniform_sample, :]
            if sampled_type == "zone":  # 片区采样
                num_zones = 4  # 示例：将256个token分为4个片区
                zone_size = int(256 * ratio / num_zones)  # 计算每个片区的大小
                sampled_x = np.zeros((1, zone_size * num_zones, 192))  # 创建采样后的变量，初始化为全零
                for i in range(num_zones):
                    start_idx = i * zone_size  # 计算当前片区的起始索引
                    end_idx = start_idx + zone_size  # 计算当前片区的结束索引
                    sampled_x[:, start_idx:end_idx, :] = x[:, start_idx:end_idx, :]  # 进行片区采样
