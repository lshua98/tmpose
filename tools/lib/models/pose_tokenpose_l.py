...
class TokenPose_L(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.transformer = TokenPose_L_base(feature_size=[cfg.MODEL.IMAGE_SIZE[1]//4,cfg.MODEL.IMAGE_SIZE[0]//4],patch_size=[cfg.MODEL.PATCH_SIZE[1],cfg.MODEL.PATCH_SIZE[0]],
                            ...
                            pos_embedding_type=cfg.MODEL.POS_EMBEDDING_TYPE,sample_type = cfg.MODEL.SAMPLED_TYPE, sample_ratio = cfg.MODEL.SAMPLED_RATIO)
        ###################################################3
...
class TokenPose_L_T(nn.Module):

    def __init__(self, cfg, **kwargs):

        extra = cfg.MODEL.EXTRA

        super(TokenPose_L_T, self).__init__()
...
def get_pure_trans(cfg, **kwargs):
    model = TokenPose_L_T(cfg, **kwargs)
    return model
